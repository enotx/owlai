// owlai/frontend/src/components/data/data-panel.tsx
...
import LocalAssetsTab from "./local-assets-tab";
import CloudHubTab from "./cloud-hub-tab";
  ...
  return (
    <div className="flex h-full flex-col border-l bg-background">
      ...
      <div className="flex items-center border-b">
        <div className="flex">
          {(
            [
              ...
              { id: "local-assets", label: "Local Assets" },
              { id: "cloud-hub", label: "Cloud Hub" },
            ] as const
          ).map((tab) => {
            ...
            return (
              <button
                ...
                style={{
                  color: isActive ? "var(--owl-tab-active-fg)" : "var(--owl-tab-inactive-fg)",
                }}
              >
                {tab.label}
                {isActive && (
                  <div className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: "var(--owl-tab-active-border)" }} />
                )}
              </button>
            );
          })}
        ...
        {/* Export button (only on inspector tab with data) */}
        <div className="ml-auto flex items-center gap-1 pr-4">
          {activeTab === "inspector" && hasData && !isTextPreview && (
                  ...
                  <TableHeader>
                    <TableRow className="bg-muted/50 hover:bg-muted/50">
                      {previewColumns.map((col) => (
                        <TableHead key={col} className="h-9 whitespace-nowrap px-3 text-xs font-semibold">
                          {col}
                        </TableHead>
                      ))}
                    </TableRow>
                  ...
                  <TableBody>
                    {previewData.map((row, idx) => (
                      <TableRow key={idx} className="hover:bg-muted/30">
                        {previewColumns.map((col) => (
                          <TableCell key={col} className="whitespace-nowrap px-3 py-2 text-xs tabular-nums">
      ...
      {activeTab === "local-assets" && <LocalAssetsTab />}
      {activeTab === "cloud-hub" && <CloudHubTab />}
    </div>
  );
}